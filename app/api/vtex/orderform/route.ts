...
export async function GET(request: NextRequest) {
  ...
  const VTEX_API_KEY = process.env.VTEX_API_KEY || process.env.VTEX_APP_KEY
  const VTEX_API_TOKEN = process.env.VTEX_API_TOKEN || process.env.VTEX_APP_TOKEN
  
  console.log("gggg", VTEX_ACCOUNT)
  console.log("Environment variables:", {
    VTEX_ACCOUNT,
    VTEX_ENVIRONMENT,
    VTEX_API_KEY: VTEX_API_KEY ? "Found" : "Missing",
    VTEX_API_TOKEN: VTEX_API_TOKEN ? "Found" : "Missing"
  })
  ...
  return NextResponse.json({
    ...
    hasApiKey: !!VTEX_API_KEY,
    hasApiToken: !!VTEX_API_TOKEN,
    apiKeyStartsWith: VTEX_API_KEY ? VTEX_API_KEY.substring(0, 20) + "..." : "Not found",
    apiTokenStartsWith: VTEX_API_TOKEN ? VTEX_API_TOKEN.substring(0, 20) + "..." : "Not found"
  })
}
  ...
  try {
    ...
    if (!hasVTEXCredentials) {
      ...
      return NextResponse.json({ 
        ...
        details: "Please set VTEX_APP_KEY and VTEX_APP_TOKEN environment variables. See VTEX_SETUP.md for instructions.",
        config: {
          account: VTEX_CONFIG.accountName,
          environment: VTEX_CONFIG.environment,
